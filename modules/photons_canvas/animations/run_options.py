from photons_canvas.animations.infrastructure.register import resolve, AnimationType
...
class RunOptions(dictobj.Spec):
    ...
    animations = dictobj.Field(
        ...
        These are a list of {[type]:name, background, options}

        ``[type]:name``
            For example, if you say "twinkles" then it'll use the twinkles
            feature animation as a feature. But if you were to say
            ``transition:twinkles`` then it'll use it as a transition. The same
            goes for transition animations you want to use as a feature, for
            example ``feature:swipe``. The animation will default to the type
            it's defined as.

            This makes a difference in the order of animations and also the
            default background option.

        background
            What the background of the animation should be

            * clear - Set a blank background for the animation
            * maintain - Use the current background for the whole animation
            * maintain:red - Use a red background for the whole animation
            * as_start - Use the current background as the start of the animation
            * as_start:green - Use a green background as the start of the animation
    ...
    def animations_iter(self, register=None):
        ...
        for a in self.animations:
            typ, make_animation, background = a.resolve()
            if typ is AnimationType.TRANSITION:
    ...
    if isinstance(val, list):
        ...
        val = {
            "animations": [
                ...
                #
                ["time", {"num_seconds": 5, "full_height_progress": True}],
                ["swipe"],
                #
