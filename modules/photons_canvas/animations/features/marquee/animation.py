...
class MarqueeAnimation(Animation):
    ...
    def maybe_switch(self, event):
        if self.switch_directions:
            if self.options.direction is MarqueeDirection.LEFT:
                ...
                return self.next_state_right(None, event.coords)
            else:
                self.options.direction = MarqueeDirection.LEFT
                return self.next_state_left(None, event.coords)
        else:
            return event.state
    ...
    def next_state_left(self, event):
        ...
        if state.x < -state.text_width:
            self.iteration += 1
            if self.options.final_iteration(self.iteration):
                raise Finish("Reached max iterations")
            event.state = None
            return self.maybe_switch(event, self.next_state_left(event))

        event.state = state.move_left(getattr(self.options, "speed", 1))
    ...
    def next_state_right(self, event):
        ...
        if state.x > width:
            ...
            if self.options.final_iteration(self.iteration):
                raise Finish("Reached max iterations")
            event.state = None
            return self.maybe_switch(event, self.next_state_right(event))

        event.state = state.move_right(getattr(self.options, "speed", 1))
