...
class Coords:
    ...
    def add_device(self, serial, chain, debug=False):
    ...
    def all_points(self):
        for coord in self:
            for j in range(coord.top_y, coord.top_y - coord.height, -1):
...
class Coord:
    ...
    @property
    def points(self):
        for j in range(self.top_y, self.top_y - self.height, -1):
